...
unsigned long long hashh(const Mystring &s){
    ...
    int main()
    {
        ...
        BPlusTree<pair<unsigned long long, int>, int, 100, 100>A("datafile","treefile",false);
        int T;cin>>T;
        while(T--){
            ...
            if(s=="insert"){
                cin>>a>>b;
                A.insert(make_pair(hashh(Mystring(a)),b), b);
            }
            if(s=="delete"){
                cin>>a>>b;
                A.remove(make_pair(hashh(Mystring(a)), b));
            }
            if(s=="find"){
                cin>>a;//cout<<"|||";
                vector<int>v;
                A.searchall(make_pair(hashh(Mystring(a)), 0), make_pair(hashh(Mystring(a)), 1000000),v);
            ...
            // int res;
            // A.search(make_pair(Mystring(string("CppPrimer")), 2012), res);
            // cout<<res<<'\n';
            // A.printconfig();
            // vector<int>v;
            // A.searchall(make_pair(Mystring("Dune"), 0), make_pair(Mystring("Dune"), 1000000),v);
