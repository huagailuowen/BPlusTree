...
#define DEBUG
// namespace sjtu  {
template <class key_t, class val_t, int M, int L> class BPlusTree {
#ifdef DEBUG
public:
#endif
  class dataNode {
...
public:
  ...
  std::shared_mutex configmutex_root, configmutex_queue, iomutex_inner,
      iomutex_data;
...
public:
    ...
    while (h <= t) {
      ...
      for (int i = 0; i < nw->num; i++) {
        ...
        if (!nw->isLeaf) {
          st[++t] = nw->children[i];
          test[nw->children[i]] =
              (i + 1 == nw->num) ? test[nw->id] : nw->keys[i];
  ...
  void print_tree() {
    // config Config;
    // getconfig(Config);
    std::unique_lock<std::shared_mutex> lck_root(configmutex_root);
    // std::unique_lock<std::shared_mutex > lck_queue(configmutex_root);
    ...
    while (h <= t) {
      int u = st[h++];
      // buffergettreefile(nw,u);
      std::shared_lock<std::shared_mutex> nwlck(treeblockmutex[u]);
      innernode_ptr nw(u, this);
      nw->print();
      for (int i = 0; i < nw->num; i++) {
        if (!nw->isLeaf) {
          st[++t] = nw->children[i];
        } else {
          std::shared_lock<std::shared_mutex> datalck(
              datablockmutex[nw->children[i]]);
...
#endif
  BPlusTree(std::string datafile, std::string treefile, bool isnew = false)
      : datafile(datafile), treefile(treefile) {
    time_stamp = 0;
    treepos.clear();
    datapos.clear();
    while (treebufferqueue.size())
      treebufferqueue.pop();
    while (databufferqueue.size())
      databufferqueue.pop();
    while (treevacantqueue.size())
      treevacantqueue.pop();
    while (datavacantqueue.size())
      datavacantqueue.pop();
    for (int i = 0; i < buffersize; i++) {
    ...
    if (isnew || file.tellp() == 0) {
      ...
      file.write(reinterpret_cast<char *>(&headtreenode),
                 sizeof(innerTreeNode));
      file.close();
  ...
  ~BPlusTree() {
    ...
    // std::unique_lock<std::shared_mutex>lck2(iomutex_data);
    // std::unique_lock<std::shared_mutex>lck3(configmutex_root);
    // std::unique_lock<std::shared_mutex>lck4(configmutex_queue);
  ...
  void checkfilesize(int pos, bool is_tree) {
    ...
    // std::cerr<<(is_tree?"tree":"data")<<fsize<<" "<<pos<<"
    // "<<blocksize<<std::endl; getchar();
    if (fsize < sizeof(config) * is_tree + pos * blocksize) {
      int times =
          (sizeof(config) * is_tree + (pos + 1) * blocksize - fsize - 1) /
          blocksize;
  ...
  void buffergettreefile(innerTreeNode *&node, int pos) {
    ...
    if (treepos.find(pos) == treepos.end()) {
      if (treevacantqueue.empty()) {
        ...
        // exit(0);
        // std::cerr<<pos<<"
        // "<<treebufferqueue.top().second<<'|'<<treebufferqueue.size()<<"tree:\n";
      ...
      treevacantqueue.pop();
      treepos[pos] = ppos;
      treebufferqueue.push(
          std::make_pair(-(treetime[pos] = ++time_stamp), pos));
      // 注记：大根堆
  ...
  void buffergetdatafile(dataNode *&node, int pos) {
    ...
    if (datapos.find(pos) == datapos.end()) {
      if (datavacantqueue.empty()) {
        // std::cerr<<pos<<"
        // "<<databufferqueue.top().second<<'|'<<databufferqueue.size()<<"data:\n";
        while (true) {
      ...
      datavacantqueue.pop();
      datapos[pos] = ppos;
      databufferqueue.push(
          std::make_pair(-(datatime[pos] = ++time_stamp), pos));
  ...
  public:
    ...
    innernode_ptr(int id, BPlusTree *tree)
        : id(id), innernode(nullptr), tree(tree) {}
    innernode_ptr(const innernode_ptr &other)
        : id(other.id), innernode(other.innernode), tree(other.tree) {}
  ...
  public:
    ...
    datanode_ptr(int id, BPlusTree *tree)
        : id(id), datanode(nullptr), tree(tree) {}
    datanode_ptr(const datanode_ptr &other)
        : id(other.id), datanode(other.datanode), tree(other.tree) {}
  ...
  bool search(const key_t &key, val_t &value) {
    ...
    std::shared_lock<std::shared_mutex> config_rootlck(configmutex_root);
    ...
    while (!nw->isLeaf) {
      ...
      std::shared_lock<std::shared_mutex> newtreelck(
          treeblockmutex[nw->children[i]]);
      nw.get(nw->children[i]);
      treelck = std::move(newtreelck);
      if (config_rootlck.owns_lock())
        config_rootlck.unlock();
  ...
  void searchall(const key_t &lower_bound, const key_t upper_bound,
    ...
    while (datanw->id != Config.dataend) {
      ...
      std::shared_lock<std::shared_mutex> newdatalck(
          datablockmutex[datanw->next]);
  ...
  void insert(const key_t &key, const val_t &value) {
    ...
    //!!这里还有一个重大bug导致死锁:
    //!我们要知道此时此刻的root是谁，所以我们需要一个锁
    // 然而，我们在这里加锁则后面有一个部分，我们另一个拥有root块的锁的线程需要这个修改root的锁
    // 但是双方各站资源互不相让，造成死锁
    // 因此，config必须要在跟释放后才能使释放
    std::unique_lock<std::shared_mutex> config_rootlck(configmutex_root);

    treelck[t++] =
        std::unique_lock<std::shared_mutex>(treeblockmutex[Config.root]);
    innernode_ptr nw(Config.root, this);
    father[nw->id] = -1;
    // nw->print();
    while (!nw->isLeaf) {
      ...
      treelck[t++] =
          std::unique_lock<std::shared_mutex>(treeblockmutex[nw->children[i]]);
      nw.get(nw->children[i]);
      if (nw->num < M) {
        while (h + 1 < t) {
          treelck[h++].unlock();
        ...
        if (h > 0)
          have_root = false;
        if (!have_root && config_rootlck.owns_lock())
          config_rootlck.unlock();
      }
      // buffergettreefile(nw,nw->children[i]);
    }
    ...
    // dataNode* datanw;
    // buffergetdatafile(datanw,nw->children[i]);
    std::unique_lock<std::shared_mutex> datalck(
        datablockmutex[nw->children[i]]);
    datanode_ptr datanw(nw->children[i], this);
    ...
    std::unique_lock<std::shared_mutex> config_queuelck(configmutex_queue);
    if (Config.vacantdatablock != -1) {
      // buffergetdatafile(newdatanw,Config.vacantdatablock);
      newdatalck = std::unique_lock<std::shared_mutex>(
          datablockmutex[Config.vacantdatablock]);
      ...
      newdatalck =
          std::unique_lock<std::shared_mutex>(datablockmutex[++Config.datacnt]);
    ...
    while (!flag) {
      ...
      if (Config.vacanttreeblock != -1) {
        ...
        //  newtreelck=
        //  std::unique_lock<std::shared_mutex>(treeblockmutex[Config.vacanttreeblock]);
        ...
        //  newtreelck=
        //  std::unique_lock<std::shared_mutex>(treeblockmutex[++Config.treecnt]);
  ...
  void remove(const key_t &key) {
    ...
    if (place > 0) {
      ...
      std::unique_lock<std::shared_mutex> prevdatalck(
          datablockmutex[datanw->prev]);
    ...
    if (place < nw->num - 1) {
      ...
      std::unique_lock<std::shared_mutex> nextdatalck(
          datablockmutex[datanw->next]);
    ...
    while (true) {
      ...
      for (place = 0; place < parent->num && parent->children[place] != nw->id;
           place++)
        ;
      // 借用节点
      if (place > 0) {
        ...
        std::unique_lock<std::shared_mutex> prevdatalck(
            datablockmutex[parent->children[place - 1]]);
